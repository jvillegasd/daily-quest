...
export async function POST(request: Request) {
  ...
  const text = `${profile.displayName} invited you to join ${household.name} on Daily Quest. Accept the invitation: ${inviteUrl}`
  const html = `
    <div style="margin:0;padding:32px 16px;background:#fdf6e3;font-family:Nunito,Arial,sans-serif;color:#3b2a14;">
      <div style="max-width:560px;margin:0 auto;background:#fff8e7;border:1px solid #e8d5a3;border-radius:18px;overflow:hidden;box-shadow:0 8px 32px rgba(100,70,20,.16);">
        ...
        <div style="padding:28px;text-align:center;">
          ...
          <p style="margin:24px 0 0;font-size:12px;line-height:1.5;color:#a08858;">This invite was sent to this email, but you can join with Google or email after opening the link.</p>
        </div>
      </div>
    </div>
  `
