...
export const tasksService = {
  async getByHousehold(householdId: string): Promise<Task[]> {
    const successors = await db.tasks.reconcileLegacyRecurrences(householdId)
    await Promise.all(successors.map((task) => notificationsService.sendTaskAssigned(task)))
  ...
  async complete(taskId: string, userId: string): Promise<{ task: Task; levelUp: LevelUpResult | null }> {
    const { task, successor } = await db.tasks.complete(taskId, userId)
    const levelUp = await pointsService.awardForTask(task, userId)
    await notificationsService.sendTaskCompleted(task, userId)
    if (successor) await notificationsService.sendTaskAssigned(successor)
    return { task, levelUp }
  },
